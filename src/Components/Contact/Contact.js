...
function Contact() {
    ...
    const sendEmail = (e) => {
        ...
        emailjs.sendForm('contact_service', 'template_l2nwp6s', form.current, 'Z3rh0mNjILy5GvJ3-')
            .then((result) => {
                setToggleModalSuccess(!toggleModalSuccess);
                console.log("je clique sur le bouton envoyer", toggleModalSuccess)
                console.log(result.text);
            }, (error) => {
                setToggleModalError(!toggleModalError);
                console.log("Je clique sur le bouton envoyer mais il y a une erreur", toggleModalError);
            ...
            {toggleModalError &&
                <div className='contact-container'>
                    <div className='contact-container-modal'>
                        ...
                        <button type="button" className='contact-button' onClick={onClickToggleCloseModalError}>Fermer</button>
                    </div>
                </div>
            }
            {toggleModalSuccess &&
                <div className='contact-container'>
                    <div className='contact-container-modal'>
                        ...
                        <button type="button" className='contact-button' onClick={onClickToggleCloseModalSuccess}>Fermer</button>
                    </div>
                </div>
            }
        </form>
